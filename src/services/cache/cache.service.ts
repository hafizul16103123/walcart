...
export async function updateSingleCategoryCache(singleDoc: any) {
...
export async function updateActiveCategoryListCache() {
    const DEFAULT_EXPIRATION = config.defaultCashExpiration
    const resp = await findAll({ model: Category, obj: {active:true}, projection: { name: 1, isRoot: 1, leaf: 1, active: 1 } });
    await redisClient.setEx('active_categories', DEFAULT_EXPIRATION, JSON.stringify(resp));
  
}
export async function updateDeactiveCategoryListCache() {
    const DEFAULT_EXPIRATION = config.defaultCashExpiration
    const resp = await findAll({ model: Category, obj: {active:false}, projection: { name: 1, isRoot: 1, leaf: 1, active: 1 } });
    await redisClient.setEx('deactive_categories', DEFAULT_EXPIRATION, JSON.stringify(resp));
